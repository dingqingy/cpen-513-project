...
class SimulatedAnnealing : public SearchAlgorithm
{
 ...
 public:
  ...
  {
    (void) config;
    // TODO: get init_temp_, max_iter_, early_stop_iter_, cooling_iter_, beta_ (optionally)
    // from config
    init_temp_ = 1000;
    max_iter_ = 1000000;
  ...
  void Report(Status status, double cost = 0)
  {
    ...
    bool mapspace_remaining = true; // keep searching until the termination condition
